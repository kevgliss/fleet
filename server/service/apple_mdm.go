...
func (svc *MDMAppleCheckinAndCommandService) handleRefetchDeviceResults(ctx context.Context, host *fleet.Host, cmdResult *mdm.CommandResults) (*mdm.Command, error) {
	...
	// SupplementalOSVersionExtra is an optional field that may contain additional information about the OS version, such as beta or build information. We want to include this in the OS version if it's present and valid, but we need to validate it first to avoid issues with excessively long or malformed strings.
